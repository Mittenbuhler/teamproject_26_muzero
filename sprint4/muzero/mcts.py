"""Vanilla latent MuZero MCTS with one recurrent inference per simulation."""
...
class MinMaxStats:
    """Track observed tree values for MuZero's Q normalization."""
...
class MCTSNode:
    """A state node whose children hold the incoming action-edge statistics."""
    ...
    # MuZero paper notation, exposed for tests and diagnostics.
...
class ModelBasedMCTS:
    ...
    def _predict(self, state):
        if self.prediction_network is not None:
            return self.prediction_network.predict(state)
        return (
            self.policy_network.action_probs(state),
            self.value_network.value(state),
        )

    def _expand_priors(self, node, priors):
        priors = np.asarray(priors, dtype=np.float64)
        if priors.shape != (self.action_dim,) or not np.isfinite(priors).all():
            raise ValueError(
                f"prediction priors must be finite with shape ({self.action_dim},)"
            )
        prior_sum = float(priors.sum())
        if prior_sum <= 0 or np.any(priors < 0):
            raise ValueError("prediction priors must be nonnegative with positive sum")
    ...
    def search(self, root_state, add_exploration_noise=False):
        root_priors, root_value = self._predict(root_state)
        root = MCTSNode(state=np.asarray(root_state, np.float32))
        self._expand_priors(root, root_priors)

        if add_exploration_noise:
            noise = np.random.dirichlet(
                [self.root_dirichlet_alpha] * self.action_dim
            )
            for action, child in root.children.items():
                child.prior = (
                    (1 - self.root_exploration_fraction) * child.prior
                    + self.root_exploration_fraction * noise[action]
                )
        ...
        for _ in range(self.simulations):
            ...
            path = [root]
            while node.expanded():
                _, node = self._select(node, stats)
                path.append(node)
                if node.state is None:
                    break

            if node.state is None:
                node.state, node.reward = self.dynamics_model.predict(
                    node.parent.state,
                    node.action,
                )
                leaf_priors, leaf_value = self._predict(node.state)
                self._expand_priors(node, leaf_priors)
                self.expansions_last_search += 1
            else:
                # This happens only for a zero-depth/root leaf.
    ...
    def _select(self, node, stats):
        ...
        for action, child in node.children.items():
            pb_c = (
                math.log(
                    (node.visit_count + self.pb_c_base + 1) / self.pb_c_base
                )
                + self.pb_c_init
            )
            pb_c *= math.sqrt(max(node.visit_count, 1)) / (child.visit_count + 1)
            q_value = (
                child.reward + self.discount * child.mean_value
                if child.visit_count
                else 0.0
            )
            score = stats.normalize(q_value) + pb_c * child.prior
...
def visit_count_policy(root, temperature=1.0):
    counts = np.asarray(
        [root.children[action].visit_count for action in range(len(root.children))],
        np.float64,
    )
    if counts.sum() == 0:
        return np.ones(len(counts), np.float32) / len(counts)
    if temperature <= 0:
        policy = np.zeros(len(counts), np.float32)
        policy[int(np.argmax(counts))] = 1
        return policy
