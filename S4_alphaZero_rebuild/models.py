...
class DynamicsModel(nn.Module):
    """Predicts next vector state and immediate reward from state + action."""
...
class ImageRepresentationNetwork(nn.Module):
    """
    CNN adapter for future screenshot observations.

    It is not used by the vector CartPole demo yet, but the policy/value/dynamics
    boundary is designed so this module can map image observations to latent
    vector states later. This architecture expects stacked 32x32 observations:
    4 input channels become a 32-dimensional latent state.
    """
